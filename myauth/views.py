...
class UserAddView(AdminUserRequiredMixin,TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        nickname = request.POST.get("row[nickname]")
        email = request.POST.get("row[email]")
        mobile = request.POST.get("row[mobile]")
        is_admin = request.POST.get("row[is_admin]")
        is_active = request.POST.get("row[is_active]")
...
class UserEditView(AdminUserRequiredMixin,TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        #根据id得到数据
        ormdata = self.model.objects.get(id=ids)
        self.id = ormdata.id
...
class UserDelView(AdminUserRequiredMixin,TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        self.model.objects.filter(id__in=ids).update(is_active=0)
        data={"code":1,"msg":msg}
