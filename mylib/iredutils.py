...
def generate_ssha512_password(p):
    ...
    try:
        ...
        pw = sha512(bytes(p,'utf-8'))
        pw.update(salt)
        #pw.update(('%s%s' % (salt, p)).encode('utf-8'))
        
        #字节转换成字符串
        password = (b64encode(pw.digest() + salt)).decode("utf-8")

        return "{SSHA512}" + password
    except ImportError:
        # Use SSHA password instead if python is older than 2.5.
        return generate_ssha_password(p)
...
def getNewVersion(urlOfXML):
    ...
    try:
        ...
        dom = parseXMLString(urllib.urlopen(urlOfXML).read())
